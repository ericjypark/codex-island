...
Chord 49
foreach($i in 1..3){Chord 50;if((Get-Content $statePath -Raw|ConvertFrom-Json).page -ne 1){throw 'Cost page did not open.'};Chord 49}
Chord 50
Wait-UI 400
...
Copy-Item $trace (Join-Path $artifacts 'frame-trace.json')
$data=Get-Content $trace -Raw|ConvertFrom-Json
foreach($kind in @('island','dollar-glow','frame')){
 $samples=@($data.samples|Where-Object {$_.Kind -eq $kind -and ($kind -eq 'dollar-glow' -or ($_.Page -eq 1 -and $_.Progress -lt 1))}|Sort-Object Milliseconds)
 if($samples.Count -gt 0){[pscustomobject]@{kind=$kind;count=$samples.Count;median=$samples[[int]($samples.Count*.5)].Milliseconds;p95=$samples[[Math]::Min($samples.Count-1,[int]($samples.Count*.95))].Milliseconds;max=$samples[-1].Milliseconds;build=$data.build}|ConvertTo-Json}
}
