...
| `hi` | `README.hi.md` | Hindi for first-pass India coverage. Keep common developer terms such as token, provider, and refresh when the Hindi alternative is less natural. |
...
| Provider | 服务 | 服務 | Provider | プロバイダ | 공급자 | Anbieter | Fournisseur | Proveedor | Provedor |
| Token | Token | Token | टोकन | トークン | 토큰 | Token | token | token | token |
| Notch | 刘海 | 劉海 | notch | ノッチ | 노치 | Notch | encoche | notch | notch |
...
| Refresh | 刷新 | 重新整理 | refresh | 更新 | 새로고침 | Aktualisieren | Actualiser | Actualizar | Atualizar |
| Syncing | 同步中 | 同步中 | Sync हो रहा है | 同期中 | 동기화 중 | Synchronisiert | Synchronisation | Sincronizando | Sincronizando |
